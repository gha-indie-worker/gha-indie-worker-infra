...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let root = manifest.parent().and_then(Path::parent).ok_or("tool must live under tools/<name>")?;
    ...
    for needle in ["MAX_BODY_BYTES", "request.arrayBuffer()", "x-ores-edge", "redirect: \"manual\"", "upstream.protocol !== \"https:\""] {
    ...
    for secret in ["INDIEBUILD_GITHUB_WEBHOOK_SECRET", "INDIEBUILD_GITHUB_STATUS_TOKEN", "INDIEBUILD_BUILD_SERVER_AUTH"] {
    ...
    for entry in fs::read_dir(root.join("cloudflare/pr-gateway"))? {
        ...
        if name.contains("secret") || name.contains("token") || name.starts_with(".env") {
            return Err(format!("credential-shaped file is forbidden in cloudflare/pr-gateway: {name}").into());
        }
    }
