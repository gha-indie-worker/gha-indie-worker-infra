...
# 1. The five historical production state boundaries are exact and remain independent.
...
for i in "${!roots[@]}"; do
  ...
  # 5. Root modules compose; provider resources/data live only in modules.
  ...
  # 7. State-address migration must point exclusively into this root's module.
...
# 12. Production module sources are exact local paths, not registry/git modules.
for i in "${!roots[@]}"; do
  root="${roots[$i]}"
  module_path="${modules[$i]}"
...
# 13. No Terraform source may import another environment, an application checkout, or build output.
...
# 15. The ORES infra manifest resolves to the canonical layout and real native authorities.
